...
#define SX_spi  ((SPI_TypeDef *) SPI1_BASE)
...
void SPI_Cmd(uint8_t data)
{
	...
	//NSS_L();
	SPI_SSOEConfig(SX_spi,DISABLE);

	SPI_SendData(SX_spi,&data,sizeof(data));
...
uint8_t SPI_Read8bit(void)
{
 ...
 //NSS_L();
 SPI_SSOEConfig(SX_spi,DISABLE);
 SPI_ReceiveData(SX_spi,&Rd_data,sizeof(Rd_data));
...
{
  uint8_t tmp;
  SPI_Cmd(adr);                                         //Send address first
  tmp = SPI_Read8bit();


  //NSS_H();

  SPI_SSOEConfig(SX_spi,ENABLE);
